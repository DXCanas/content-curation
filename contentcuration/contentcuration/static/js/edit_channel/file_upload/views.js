...
var FileModalView = BaseViews.BaseModalView.extend({
  ...
  close_file_uploader: function(event) {
    var self = this;
    if (
      this.file_upload_view.collection.length === 0 &&
      this.file_upload_view.current_view.uploads_in_progress > 0
    ) {
      this.close();
    } else {
      dialog.dialog(
...
var FileUploadView = BaseViews.BaseView.extend({
  ...
  disable_next: function(upload_in_progress) {
    ...
    this.$('.go_to_metadata').text(
      upload_in_progress
        ? this.get_translation('upload_in_progress')
        : this.get_translation('add_files')
    );
  },
...
var FileUploadList = BaseViews.BaseEditableListView.extend({
  ...
  show_uploading: function() {
    ...
    this.$(this.default_item).css(
      'display',
      this.$('.format_item').length || this.uploads_in_progress || is_checked ? 'none' : 'block'
    );
  ...
  set_uploading: function(uploading) {
    ...
    this.uploads_in_progress <= 0
      ? this.container.enable_next()
      : this.container.disable_next(this.uploads_in_progress);
  },
  ...
  file_failed: function(file, error, xhr) {
    if (xhr && xhr.status === 403) {
      // Catch errors thrown by server
      this.$('.error').html(this.get_translation('no_space'));
    }
    this.uploads_in_progress--;
    $(file.previewTemplate)
      .find('.dropzone_remove')
...
var FormatEditorItem = BaseViews.BaseListNodeItemView.extend({
  ...
  getToggler: function() {
    return this.$('.expand_format_editor');
  },
  getSubdirectory: function() {
    return this.$('.format_editor_list');
  },
  id: function() {
  ...
  init_collections: function() {
    this.presets = new Models.FormatPresetCollection(
      _.reject(this.model.get('associated_presets'), function(p) {
        return !p.display || p.thumbnail;
      })
    );
  ...
  update_metadata: function() {
    ...
    this.$('.main_file_remove').css(
      'display',
      all_metadata.main_file_count <= 1 ? 'none' : 'inline'
    );
...
var FormatSlotList = BaseViews.BaseEditableListView.extend({
  ...
  set_file_format: function(file, preset) {
    var currentFiles = this.node.get('files');

    // Remove files with the same preset
    var originalFile = _.find(currentFiles, function(file) {
      return file.preset.id === preset.id;
    });
    if (originalFile) {
      this.files.remove(originalFile);
      this.is_language && this.collection.remove(preset); // Remove languages
      this.node.set(
        'files',
        _.reject(currentFiles, function(f) {
    ...
    if (file) {
      file.set({ preset: preset.toJSON(), contentnode: this.node.id });
      this.collection.add(preset);
      this.files.push(file);
      currentFiles.push(file.toJSON());
      this.node.set('files', currentFiles);
    }
  },
  set_uploading: function(uploading) {
...
var FormatSlot = BaseViews.BaseListNodeItemView.extend({
  ...
  toggle_template: require('./hbtemplates/format_item_multilanguage_toggle.handlebars'),
  ...
  getToggler: function() {
    return this.$('#format_item_' + this.model.id);
  },
  getSubdirectory: function() {
    return this.$('#format_item_' + this.model.id + '_sub');
  },
  ...
  initialize: function(options) {
    ...
    this.is_language = options.is_language;
    this.render();
  },
  events: {
  ...
  render: function() {
    if (this.model.get('multi_language') && !this.is_language) {
      this.$el.html(
        this.toggle_template(this.model.toJSON(), {
          data: this.get_intl_data(),
        })
      );
    } else {
...
var MultiLanguageSlotList = FormatSlotList.extend({
  ...
  initialize: function(options) {
    ...
    this.files = new Models.FileCollection(
      options.files.filter(function(f) {
        return f.get('preset').name === preset_id;
      })
    );
...
var MultiLanguageUploadSlot = FormatSlot.extend({
  ...
  selector: function() {
    return this.model.get('id') + '_' + this.node.get('id') + '_dropzone';
  },
  initialize: function(options) {
    _.bindAll(
      this,
  ...
  get_unassigned_languages: function() {
    var current_languages = _.pluck(
      _.filter(this.files.pluck('language'), function(l) {
        return l;
      }),
      'id'
    );
    return new Models.LanguageCollection(
      Constants.Languages.filter(function(l) {
        return !_.contains(current_languages, l.id);
      })
    );
  },
  events: {
